-- =========================================================================
-- Migration 014 — OT rate is manual, not auto-1.5× (Item 13).
-- Run in the Supabase SQL editor. Idempotent.
--
-- Previously pay_breakdown derived an OT rate as regular_rate * 1.5 whenever
-- employees.ot_rate was null. Adèle wants OT rate entered explicitly per
-- employee (the employee form now exposes Hourly / OT / PTO rate). This drops
-- the 1.5× fallback: a null ot_rate with OT hours now surfaces as a warning
-- (same as every other missing rate), never silently multiplied.
--
-- Also makes the warnings human-readable (Item 17): "Missing OT rate" instead
-- of leaking the raw "Missing ot_rate / regular_rate" column name.
--
-- Only the two CTE/expression lines below differ from supabase/payroll.sql.
-- =========================================================================
