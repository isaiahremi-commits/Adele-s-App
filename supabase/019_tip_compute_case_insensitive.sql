-- =========================================================================
-- Migration 019 — case-insensitive position lookup in ts_compute (Day-4 item 8).
-- Run in the Supabase SQL editor. Idempotent.
--
-- VALIDATION BLOCKER: "Compute" raised "Missing points config for position(s):
-- bartender" even though Lobby Bar has bartender configured with points in
-- outlet_roles. Root cause: the engine joined outlet_roles.position_name to the
-- employee's effective position (shift.position / home_position / position)
-- with a CASE-SENSITIVE equality. Day-2's title-casing left position values in
-- mixed case across tables (e.g. outlet_roles 'Bartender' vs shifts
-- 'bartender'), so the join missed and points came back NULL.
--
-- Fix: compare positions with LOWER() on both sides. This is the only
-- DB-stored position-name comparison in the engine; the rest of the math is
-- identical to migration 013. Return shape (jsonb) is unchanged, so plain
...
begin
  ...
  -- Day-4 item 8: case-insensitive position match (was a plain = comparison).
