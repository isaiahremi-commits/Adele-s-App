...
export default function SchedulingPage() {
  ...
  const [form, setForm] = useState({ employee_id: "", shift_date: "", start_time: "09:00", end_time: "17:00", role: "" });
  ...
  async function addShift(e: React.FormEvent) {
    ...
    if (res.ok) {
      setModalOpen(false);
      setForm({ employee_id: "", shift_date: "", start_time: "09:00", end_time: "17:00", role: "" });
      load();
    }
  }
                      ...
                      <div className="flex flex-col gap-1">
                        {list.map((s) => (
                          <div key={s.id} className="p-2 rounded-md text-xs group relative" style={{ background: "var(--surface-2)", border: "1px solid var(--border)" }}>
                            ...
                            {s.role && <div style={{ color: "var(--muted)" }}>{s.role}</div>}
                            <button
                        ...
                        <button
                          onClick={() => { setForm({ ...form, employee_id: emp.id, shift_date: toISODate(d) }); setModalOpen(true); }}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Add Shift">
        <form onSubmit={addShift} className="flex flex-col gap-3">
          <label className="text-sm">Employee
            <select className="input mt-1" required value={form.employee_id} onChange={(e) => setForm({ ...form, employee_id: e.target.value })}>
          ...
          <label className="text-sm">Role
            <input type="text" className="input mt-1" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} placeholder="Server, Host, Bartender…" />
          </label>
